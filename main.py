...
class ItemsSender:
    ...
    def run(self):
        ...
        try:
            ...
            telegram_notify(self.chat_id, self.token, f'Critical error: {format_exc()}')

        while True:
            try:
                while True:
                    if stop_flag:
                        message(self.login, 'y', 'Exit from sender')
                        return

                    message(self.login, 'y>', 'Checking for new trades..')
                    offers = check_active_offers(self.tm_api)
                    if not offers:
                        message(self.login, 'y>', 'No active offers')
                        sleep(30)
                        continue
                    ...
                    try:
                        ...
                        message(self.login, 'r', 'Some error in creating offers')

                    message(self.login, 'y>', 'Sent all offers!')
                    sleep(30)
            except:
                telegram_notify(self.chat_id, self.token, f'Critical error in Sender module {self.login}')
                print(format_exc())
    ...
    def get_my_steam_api(self):
        ...
        try:
            ...
            message(self.login, 'r', f'Error getting your steam api')
            stop_flag = True
    ...
    def create_single_offer(self, offer):
        ...
        return self.session.post(create_offer_link, data=data, headers=headers, timeout=60)

    def confirm_all_trade_offers(self):
        for i in range(3):
            try:
                message(self.login, 'y', 'Confirming..')
    ...
    def login_to_account(self):
        ...
        message(self.login, 'y>', 'Logining to account..')
        try:
            self.session = LoginExecutor(self.login, self.password, self.shared_secret).run()

        except:
            telegram_notify(self.chat_id, self.token, f'Unexpected error in login to {self.login}')
            message(self.login, 'r', 'Unexpected error in login')
            sleep(60)
            return False

        else:
            message(self.login, 'n', 'Success login')
            telegram_notify(self.chat_id, self.token, f'Success login to {self.login}')
